...
def find_goal_point(path, current_pos, lookahead_distance, last_found_index):
    ...
    for i in range(starting_index, len(path) - 1):
        ...
        if discriminant >= 0:
            ...
            sol_pt1 = [sol_x1 + current_pos[0], sol_y1 + current_pos[1]]
            sol_pt2 = [sol_x2 + current_pos[0], sol_y2 + current_pos[1]]
            ...
            is_sol1_in_range = (minX <= sol_pt1[0] <= maxX) and (
                minY <= sol_pt1[1] <= maxY
            )
            is_sol2_in_range = (minX <= sol_pt2[0] <= maxX) and (
                minY <= sol_pt2[1] <= maxY
            )
            ...
            # case 4: both points in range => select point closest to last path point
                ...
                goal_point = (
                    sol_pt1
                    if pt_to_pt_distance(sol_pt1, path[i + 1])
                    < pt_to_pt_distance(sol_pt2, path[i + 1])
                    else sol_pt2
                )

            if intersection_found and pt_to_pt_distance(
                goal_point, path[i + 1]
            ) < pt_to_pt_distance(current_pos, path[i + 1]):
